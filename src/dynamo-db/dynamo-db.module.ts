...
import { IConnection, IConnectionAsyncOptions, IConnectionOptions } from "./dynamo-db.interfaces";
...
export class DynamodbModule {
    ...
    private static makeConnection(opts: IConnectionOptions): IConnection {
        if (!this.client) {
            this.client = {
                db: new DynamoDB({ region: opts.region, credentials: opts.credentials }),
    ...
    static forFeature(cls: new () => any, connectionName?: string): DynamicModule {
        const providers: Provider[] = [
            {
